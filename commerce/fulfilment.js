...
//
// Public exports:
//   applyPaymentState(env, database, order, payment)
//   deliverOrder(env, database, order)
//   resendLicense(env, database, order)
//   reconcile(env, database)
//   alertAdmin(env, database, title, lines, orderId)
//
// This is the file that owes the customer something. Every
// other part of the checkout can fail and be retried
// invisibly; from the moment a payment confirms, there is
// money on our side and nothing on theirs, and every branch
// below is written from that position.
//
...
//
// Interrupted anywhere, the order sits in a state the
// reconciler recognises and resumes from - which is why the
// steps write their results down as they go rather than at
// the end. A crash after minting must never mint again: the
// customer would hold two keys, one of which we would later
// have to revoke.
// ==========================================
...
//
// Signed, so the link in an email cannot be edited into
// somebody else's order. Built here rather than passed in,
// because it appears in the key email, the receipt and the
// admin alert, and three call sites building the same URL
// is three chances to leave the signature off one of them.
// ==========================================
...
// concurrently. Everything that must happen once is behind
// claimForIssuing, and everything after it is either
// idempotent or guarded by what the row already holds.
// ==========================================
...
//
// The single place a provider status turns into a decision,
// so the webhook and the reconciler cannot disagree about
// what "partially_paid" does - they call this and get the
// same answer.
// ==========================================
...
//
// The same key rather than a fresh one, which is the whole
// reason the sealed copy exists. A resend that minted a new
// key would silently invalidate whichever one the customer
// had already activated, and they would discover it the
// next time Unity renewed.
//
// Rate limited by counting what this order has already
// sent. The button is public - it needs to be, because the
// person pressing it has lost their email - and a public
// button that sends mail is a public button that sends mail
// forever if nothing counts.
// ==========================================
...
//
// Best-effort, and never allowed to break the path it is
// called from. An alert that throws inside a delivery
// failure would replace a recoverable problem with an
// unrecoverable one - and the operator would get neither
// the alert nor the order.
//
// Queued through the outbox like any other message, so an
// alert raised while the mail provider is down is itself
// retried rather than lost. Which is the case where it
// matters most.
// ==========================================
...
//
// By payment id when we have one, and by invoice otherwise.
// The two are not interchangeable: an order knows its
// invoice from the moment it is opened, and only learns a
// payment id once the customer has picked a coin - so an
// order that was never paid has no payment to ask about,
