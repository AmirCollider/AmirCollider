...
// claiming to be one would render a card on the dashboard and
// be unable to sign a single player in. So the "add a game"
// screen writes the source and asks you to paste and deploy,
// and config.js stays the honest answer to "which games exist".
//
// The SQL builder is the same refusal in a different shape: it
// writes the migration, it never executes it. A panel that
// could run generated SQL against a bound database is one
// stolen cookie away from dropping a live game's players table.
//
// ------------------------------------------------------------
// THE PASSWORD
// ------------------------------------------------------------
// TestSitePassword — the same one the test panel uses, as asked
// for. Its own cookie, scoped Path=/thegod, so a browser simply
// does not send it anywhere else: signing into one panel does
// not silently hand the other one to a page that borrowed the
// tab.
// ==========================================
...
function renderLogin(lang, theme, failed) {
  ...
  return `<!DOCTYPE html>
<html lang="${esc(lang)}" dir="${t.dir}"${themeAttr}>
<head>
...
<body>
  <div class="login">
    <div class="card" style="text-align:center">
      ...
      <h1 style="font-size:1.25em;font-weight:800;margin-block-end:6px">${esc(t.loginTitle)}</h1>
      <p class="muted" style="font-size:.85em;margin-block-end:22px">${esc(t.loginSub)}</p>

      ${failed ? `<div class="note err" style="text-align:start">${esc(t.loginError)}</div>` : ''}

      <form method="POST" action="/thegod/login">
        <label class="f" style="text-align:start">
          <span>${esc(t.loginPassword)}</span>
          <input type="password" name="password" id="pw" required autofocus
                 placeholder="${esc(t.loginPlaceholder)}" autocomplete="current-password">
        </label>
        <label class="row" style="font-size:.8em;color:var(--dim);margin-block-end:16px;cursor:pointer">
          <input type="checkbox" onchange="document.getElementById('pw').type=this.checked?'text':'password'"
                 style="width:auto">
          <span>${esc(t.showPassword)}</span>
        </label>
        <button type="submit" class="btn" style="width:100%">${esc(t.loginButton)}</button>
      </form>
    </div>
  </div>
</body>
...
// Panel page
//
// The whole game list is server-rendered into the document as
// JSON rather than fetched after paint. It is a few kilobytes,
// it is already in memory on the server, and a panel that
// flashes empty for 300ms every load is a panel that feels
...
function renderPanel(games, lang, theme, health, origin) {
  ...
  const healthPills = [
    ...
    `<span class="hpill${pill.on ? ' is-on' : ''}"><span class="hdot"></span>${esc(pill.label)}: ${esc(pill.on ? t.hOn : t.hOff)}</span>`
  ).join('')

  const sandboxPill = health.sandbox
    ? `<span class="hpill is-warn"><span class="hdot"></span>${esc(t.hSandbox)}</span>` : ''

  const langSeg = LANGS.map(code =>
    `<button type="button" onclick="tgLang('${code}')" aria-pressed="${code === lang ? 'true' : 'false'}">${esc(META[code].label)}</button>`
  ).join('')
  ...
  const tabs = [
    ...
    `<button type="button" role="tab" data-tab="${key}" aria-selected="${index === 0 ? 'true' : 'false'}"
             onclick="tgTab('${key}')">${icon} ${esc(label)}</button>`
  ).join('')
...
<body>
  <div class="wrap">
    <div class="top">
      ...
      <div class="top-actions">
        ...
        <form method="POST" action="/thegod/logout" style="display:inline">
          <button type="submit" class="btn ghost small">${esc(t.logout)}</button>
        </form>
      </div>
    </div>
    ...
    ${health.sandbox ? `<div class="note warn">${esc(t.hSandboxWarn)}</div>` : ''}
    ${!health.database ? `<div class="note err">${esc(t.hDbWarn)} ${esc(t.hNoMigration)}</div>` : ''}
...
// Client runtime
//
// Kept in one string rather than assembled from fragments,
// because the alternative - template literals inside template
// literals - is where this file would start producing markup
// that is subtly wrong in one language and fine in the other
// two.
//
// Everything that writes to the DOM goes through tgEsc. The
// values are game names and descriptions an operator typed, and
// an admin panel that renders its own input unescaped is an
// admin panel where one paste is a stored XSS with a session
