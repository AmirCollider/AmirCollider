...
export async function handleLicenseAdmin(url, request, gameId, requestId, GAMES, env) {
  ...
  if (action === 'generate') {
    ...
    const batch = sanitizeLabel(body.batch) || new Date().toISOString().slice(0, 10)
    ...
    logInfo('License batch generated', { requestId, batch, count: written })
    ...
    return ok({
      ...
      serials: keys.join('\n'),
      keys,
      note: 'These plaintext keys are shown once and are not stored. Save them before closing this response.'
    })
  }
...
<body>
  <div class="wrap">
    ...
    <section class="card muted">
      ...
      <p>Unity DocSnap Free needs no key at all — install it and every core export works. Pro is a one-off
         ${'$49.99'} that adds the AI summary outputs, the Changes page, unlimited version history,
         incremental updates, file copies, project backups and CI automation.</p>
      <a class="btn" href="/unity-docsnap">See what Pro adds</a>
    </section>
  </div>
...
function licenseScript() {
  return `
    ...
    function render(data) {
      var html = '<div class="note ok">Key <b>' + esc(data.key) + '</b> — '
        + esc(data.tier === 'pro' ? 'Pro licence' : data.tier) + '</div>';
