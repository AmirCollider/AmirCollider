...
// WHAT THIS IS FOR
// ------------------------------------------------------------
// A game sells things through whatever store it shipped in.
// That works until it does not: a player in a country the store
// does not bill in, a payment method it will not take, a build
// handed out directly. Each of those is somebody who wants to
// pay and cannot.
//
// This is the other door. Same Google account, same
// entitlements table, same answer from the game's point of
// view - it asks what the player owns and is told, without ever
// learning which door the purchase came through.
//
// The payment path is the one already proven by the licence
// checkout: a hosted NOWPayments invoice, a signed callback, a
// reconciler behind it for the callback that never arrives. The
// order table and the fulfilment are separate, because a game
// purchase reconciled by the licence checkout's cron would
// eventually be posted a Unity DocSnap key.
// ==========================================
...
import {
  esc, jsString, page, chromeLang, chromeTheme, langHeader, localeFor, safeColor
} from './gameChrome.js'
...
// GET /:gameId/store
//
// Server-rendered, including what the player already owns.
// A shelf that renders empty and then fills in after a fetch
// shows somebody a "buy" button on something they bought last
// week, for as long as the fetch takes.
// ==========================================
...
//
// The session is the only thing that says who is buying. The
// body names a product and nothing else - no price, no email,
// no player id - because every one of those, accepted from a
// request, is a way to buy a season pass for a penny or to have
// it delivered to somebody else's account.
// ==========================================
...
//
// Rendered server-side with whatever the order says right now,
// then kept current by polling. The first paint matters: this
// page is opened by somebody who has just parted with money,
// and a spinner that resolves after a fetch is a second of not
// knowing whether anything worked.
// ==========================================
...
function renderStore(game, lang, theme, { products, player, owned, ready }) {
  ...
  const accent = safeColor(game.color)
  ...
  const cards = products.map(product => {
    ...
    return `
      <article class="pcard"${product.badge === 'best' ? ' data-featured="1"' : ''}>
        ${badge ? `<span class="pbadge">${esc(badge)}</span>` : ''}
        <div class="pic">${esc(product.icon || '📦')}</div>
        <h3 class="pname">${esc(localized(product.i18n && product.i18n.name, lang, product.id))}</h3>
        <p class="pdesc">${esc(localized(product.i18n && product.i18n.description, lang, ''))}</p>

        <div class="pmeta">
          <span class="gchip is-dim">${esc(kindLabel(product.kind, t))}</span>
          ${product.durationDays ? `<span class="gchip is-dim">${Number(product.durationDays)} ${esc(t.days)}</span>` : ''}
          ${balance > 0 ? `<span class="gchip is-ok">${esc(t.have)} ${Number(balance).toLocaleString(locale)}</span>` : ''}
        </div>

        <div class="pfoot">
          <span class="pprice" dir="ltr">$${esc(product.priceUsd)}</span>
          ${isOwned
            ? `<span class="gchip is-ok">✓ ${esc(t.owned)}</span>`
            : `<button type="button" class="gbtn" data-product="${esc(product.id)}"
                 onclick="gsBuy(this)"${!player || !ready ? ' disabled' : ''}>${esc(t.buy)}</button>`}
        </div>
      </article>`
  }).join('')

  const gate = !game.capabilities.store || !products.length
    ? `<div class="gcard"><h2 style="font-size:1.05em;margin-block-end:8px">${esc(t.closed)}</h2>
         <p style="color:var(--dim);font-size:.92em">${esc(t.closedBody)}</p></div>`
    : !ready
      ? `<div class="gnote is-warn" style="margin-block-end:18px">${esc(t.off)}</div>`
      : !player
        ? `<div class="gcard" style="margin-block-end:20px;display:flex;gap:16px;align-items:center;flex-wrap:wrap">
             <span style="font-size:2em">🔐</span>
             <span style="flex:1 1 260px">
               <b style="display:block;margin-block-end:4px">${esc(t.needSignIn)}</b>
               <span style="color:var(--dim);font-size:.88em;line-height:1.6">${esc(t.needSignInBody)}</span>
             </span>
             <a class="gbtn" href="/${esc(game.id)}/account/signin?lang=${esc(lang)}&next=${encodeURIComponent('/' + game.id + '/store')}">
               ${esc(t.signIn)}</a>
           </div>`
        : ''
...
function renderOrder(game, lang, theme, handle, order) {
  ...
  if (!order) {
    const body = `
      <div class="ghead">${esc(t.orderTitle)}</div>
      <div class="gcard">
        <p style="color:var(--dim);font-size:.95em;line-height:1.7">${esc(t.notFound)}</p>
        <div style="margin-block-start:18px">
          <a class="gbtn" href="/${esc(game.id)}/store">${esc(t.toStore)}</a>
        </div>
      </div>`
  ...
  const body = `
    ...
    <div class="gcard" style="text-align:center;padding:34px 26px">
      ...
      <h1 style="font-size:1.2em;margin-block-end:10px" id="gs-title">${esc(view.h)}</h1>
      <p style="color:var(--dim);font-size:.93em;line-height:1.75;max-width:46ch;margin:0 auto"
         id="gs-body">${esc(view.b)}</p>

      <div style="display:flex;gap:10px;justify-content:center;flex-wrap:wrap;margin-block-start:24px">
        <a class="gbtn" href="/${esc(game.id)}/account?lang=${esc(lang)}">${esc(t.toAccount)}</a>
        <a class="gbtn gbtn--ghost" href="/${esc(game.id)}/store?lang=${esc(lang)}">${esc(t.toStore)}</a>
      </div>
    </div>
    ...
    <div class="gcard" style="margin-block-start:16px">
      <div style="display:flex;justify-content:space-between;gap:12px;padding-block:8px;border-block-end:1px solid var(--border)">
        <span style="color:var(--dim);font-size:.86em">${esc(t.orderNo)}</span>
        <code dir="ltr" style="font-size:.86em">${esc(order.id)}</code>
      </div>
      <div style="display:flex;justify-content:space-between;gap:12px;padding-block:8px;border-block-end:1px solid var(--border)">
        <span style="color:var(--dim);font-size:.86em">${esc(productName)}</span>
        <b dir="ltr">$${esc(order.price_usd)}${order.quantity > 1 ? ' ×' + order.quantity : ''}</b>
      </div>
      <div style="display:flex;justify-content:space-between;gap:12px;padding-block:8px">
        ...
        <span class="gchip ${view.cls === 'is-ok' ? 'is-ok' : 'is-dim'}" id="gs-state">${esc(order.status)}</span>
      </div>
      <p style="margin-block-start:14px">
        <a href="mailto:${esc(CONFIG.SUPPORT_EMAIL)}?subject=${encodeURIComponent(game.name + ' order ' + order.id)}"
           style="color:var(--dim);font-size:.84em">${esc(t.support)}</a>
      </p>
    </div>`
