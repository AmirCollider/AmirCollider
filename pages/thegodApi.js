...
// for one reason: authorisation. Every action in here needs the
// same check, and a single door means it cannot be forgotten on
// the twenty-first.
//
// ------------------------------------------------------------
// THE TWO RULES
// ------------------------------------------------------------
// 1. A game id is only ever LOOKED UP, never created. Every
//    action starts from the merged registry, so an id that is
//    not in config.js resolves to nothing and the action
//    answers 404. No body, from anybody, can add a game.
//
// 2. Nothing here executes generated SQL. The sql.* actions
//    return text. Running it is a human with wrangler, on
//    purpose - see the note in games/sql.js.
//
// Under /thegod/ rather than /api/ because the panel's session
// cookie is scoped Path=/thegod, so a browser simply does not
...
//
// Identical contract to the licence admin panel and the
// checkout simulator, and intentionally so: it is the same
// operator doing the same job from the same two places, and
// three different rules for that would be three chances to get
// one of them wrong.
// ==========================================
...
//
// Identical to what the page embeds at first paint, so a game
// re-rendered after a save and a game rendered on load cannot
// look different. Assembled here rather than in the page for
// exactly that reason.
// ==========================================
...
// Field sanitising
//
// Everything written to game_settings passes through one of
// these. The panel is trusted in the sense that it needs a
// password; it is not trusted to send a status the merge layer
// has never heard of, or a colour that is a fragment of CSS.
// ==========================================
...
export async function handleTheGodApi(url, request, gameId, requestId, GAMES, env) {
  ...
  switch (action) {
    ...
    //
    // For the case the reconciler cannot fix on its own: a
    // product removed from the catalogue between the invoice and
    // the payment, or an order that was marked paid while
    // something downstream was broken.
    //
    // Refuses anything that is not `paid`. Granting an unpaid
    // order from a web panel is the one action here that could
    // give away the product, so the guard is a state check
    // rather than a confirmation dialogue.
    // -----------------------------------------------------------
    case 'order.grant': {
      const { database, refusal: noDb } = needDatabase(env)
      if (noDb) return noDb
    ...
    //
    // Accepts an id that does not exist yet, deliberately: the
    // whole point is generating the schema for a game that is
    // about to be added. It is text on a page either way, and
