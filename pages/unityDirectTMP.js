...
//
// Served at /unity-directtmp (and /directtmp), and linked
// from inside the Unity Editor itself - the About window, the
// Settings page and the health report all point here - so a
// visitor is as likely to already have the tool installed as
// to have never heard of it. That shapes the layout:
//
...
//
// The page is painted in the tool's own Inkwell palette rather
// than the site's violet - the same six hex values that
// DirectTMPConstants.cs hands to the editor windows - so
// somebody who has seen the tool recognises the page and
// somebody who sees the page recognises the tool.
//
...
// The Inkwell palette.
//
// Duplicated here from the Unity package's
// DirectTMPConstants.cs rather than imported, because the
// two live in different repositories and a Worker cannot
// read a C# file. Six hex values is a small duplication; a
// product page painted in a colour the tool does not use is
// not.
// ==========================================
const INK = '#14808C'
...
// The feature grid.
//
// Kept as data next to the i18n pack rather than inside it,
// because a feature is one idea in three languages and
// splitting it across three objects is how the Japanese list
// ends up one item shorter than the English one.
// ==========================================
const FEATURES = [
  {
    mark: '🅰️',
...
// Inky
//
// The same mascot the package ships as Docs~/mascot.svg and
// paints into its editor windows, inlined here so the page
// has no image request to make and no asset to keep in sync
// with a second copy on disk.
// ==========================================
function inky(size) {
  return `
...
// The tofu comparison.
//
// The "broken" side is not an image and not a font trick: it
// is the same string with every letter replaced by U+25A1
// WHITE SQUARE, which is exactly what a font without those
// glyphs draws. Rendering it as literal boxes rather than
// relying on the visitor's own missing font is what makes it
// look the same on every machine - including the ones that
// have the font and would otherwise see two identical lines.
...
// ==========================================
// Client runtime
// Theme toggle, language switch, and the copy button.
// ==========================================
function getClientScript() {
  return `<script>
    (function () {
      ...
      // The clipboard API needs a secure context and a user
      // gesture, and refuses in a few embedded browsers even
      // with both. The fallback selects the URL so a person can
      // copy it themselves rather than pressing a button that
      // silently does nothing.
      function acCopy(text) {
        var btn = acById('copyBtn');
        var done = function () {
          if (!btn) return;
      ...
      window.acCopy = acCopy;

      function acSelectUrl() {
        var el = acById('gitUrl');
